...
export class Heatmap {
  ...
  add(chunkId:string,at=Date.now()) { const old=this.hits.get(chunkId);this.hits.set(chunkId,{value:(old?old.value*Math.pow(.95,(at-old.at)/60000):0)+1,at}); }
  snapshot(now=Date.now()) { return Object.fromEntries([...this.hits].map(([id,h])=>[id,h.value*Math.pow(.95,(now-h.at)/60000)])); }
}
