...
export async function ingest(
  ...
  if (chunks.length > 100)
    throw new Error(
      "Please split this document into smaller uploads (maximum 100 chunks).",
    );
  ...
  return db.$transaction(
    async (tx) => {
      const document = await tx.document.create({
        data: { roomId, filename, sourceType },
      });
      for (const { vector, ...chunk } of embedded) {
        const saved = await tx.documentChunk.create({
          data: { documentId: document.id, ...chunk },
        });
